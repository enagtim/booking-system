...
import { IRoomModel } from './dto/room.dto';

@Injectable()
export class RoomService {
	constructor(@InjectModel(RoomModel.name) private roomModel: Model<RoomDocument>) {}
	public async create(roomDto: IRoomModel): Promise<RoomModel> {
	...
	public async update(id: string, roomDto: Partial<IRoomModel>): Promise<RoomModel> {
		const room = await this.roomModel.findById(id).exec();
		if (!room) {
			throw new Error(`Room with ID ${id} not found`);
		}
